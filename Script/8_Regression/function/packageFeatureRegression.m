...
result = table('Size', [8 2], 'VariableTypes',...
        {'double','double'}, 'VariableNames', {'LSBoost','Random Forest'},...
...
for i = 1:8
	...
	% Applico il modello LSBoost e memorizzo l'RMSE
    [~,validationRMSE,~] = LSboostRegressionModel(dataset);
    result(i,1) = {validationRMSE};
	
	% Applico il modello Random Forest e memorizzo l'RMSE
    [~,validationRMSE,~] = RandomForestRegressionModel(dataset);
...
% Salvo i risultati
writetable(result,strcat(datasetName,"_ensemble_methods_package_feature",".xlsx"),'WriteRowNames',true);
