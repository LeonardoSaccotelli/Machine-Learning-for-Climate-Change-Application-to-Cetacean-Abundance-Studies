% Restituisce una rete neurale a 3 layers di regressione addestrato e il relativo RMSE. 
...
%
% Output:
% - threeLayerNeuralNetwork_Trained_Model: una struttura contenente la rete neurale di regressione addestrata. 
% 	La struttura contiene vari campi con informazioni sul modello.
%	threeLayerNeuralNetwork_Trained_Model.predictFcn: una funzione per fare previsioni su nuovi dati.
%
...
% [threeLayerNeuralNetwork_Trained_Model, validationRMSE,validationPredictions] =...
%														  = ThreeLayerNeuralNetwork (T) 
%
% Per fare previsioni con il "modello addestrato" restituito su nuovi dati T2, utilizzare
% yfit = threeLayerNeuralNetwork_Trained_Model.predictFcn (T2)
%
...
% threeLayerNeuralNetwork_Trained_Model.HowToPredict
function [threeLayerNeuralNetwork_Trained_Model, validationRMSE,validationPredictions] = ...
    ThreeLayerNeuralNetwork(trainingData)
...
threeLayerNeuralNetwork_Trained_Model.predictFcn = @(x) neuralNetworkPredictFcn(predictorExtractionFcn(x));

% Aggiungiamo campi informativi alla nostra struttura
threeLayerNeuralNetwork_Trained_Model.RequiredVariables = inputTable.Properties.VariableNames;
threeLayerNeuralNetwork_Trained_Model.RegressionNeuralNetwork = regressionNeuralNetwork;
threeLayerNeuralNetwork_Trained_Model.About = 'This struct is a trained model exported from Regression Learner R2021a.';
threeLayerNeuralNetwork_Trained_Model.HowToPredict = sprintf('To make predictions on a new table, T, use: \n  yfit = c.predictFcn(T) \nreplacing ''c'' with the name of the variable that is this struct, e.g. ''threeLayerNeuralNetwork_Trained_Model''. \n \nThe table, T, must contain the variables returned by: \n  c.RequiredVariables \nVariable formats (e.g. matrix/vector, datatype) must match the original training data. \nAdditional variables are ignored. \n \nFor more information, see <a href="matlab:helpview(fullfile(docroot, ''stats'', ''stats.map''), ''appregression_exportmodeltoworkspace'')">How to predict using an exported model</a>.');
...
partitionedModel = crossval(threeLayerNeuralNetwork_Trained_Model.RegressionNeuralNetwork, 'KFold', 5);
